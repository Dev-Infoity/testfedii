...
export const ResultDetailsStutoSchPage = () => {
    ...
    const [studentschoolresult, setstudentschoolresult] = useState([]);
    ...
    React.useEffect(
       ...
       const uuniquegradeheat = [];
       studentschoolheatmap.map(clist => {
            //var indexs = uniqueTags.findIndex(a => a.keydata === clist.Keydata);
            if (clist.Grade != "All")
            {
                if (uuniquegradeheat.indexOf(clist.Grade) === -1) {
                    uuniquegradeheat.push(clist.Grade);
                }
            }
       ...
       topicdetail.map(clist => {
            var indexs = uniqueQuestions.findIndex(a => a.question === clist.question && a.topic === clist.topic);
                if (indexs === -1) {
                    uniqueQuestions.push({question : clist.question, sno : clist.questionnum,topic : clist.topic});
                }
            ...
            <Modal.Body className="cstmmdlinfodv2 cstmmdlinfodv2cstmm p-0 m-0">
                ...
                <div className="infomdvmdl1 rsltmdltbmaindv1 col-sm-12 p-0">
                    <Tabs className="row m-0">
                        ...
                        {uniqueTopics.map((unt)=>(
                            <TabPanel>
                                {/* {topicdetail.map((tpcd)=>{
                                    if(unt.topic == tpcd.topic)
                                    {
                                return( */}
